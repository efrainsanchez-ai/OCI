...
' > "$target_json"
  ...
  if ok "$target_ocid"; then
    ...
    if ok "$FSS_FILE_SYSTEM_OCID" && ok "$export_set_ocid"; then
      export_ocid="$(
        oci fs export list           --region "$REGION"           --compartment-id "$POC_COMPARTMENT_OCID"           --file-system-id "$FSS_FILE_SYSTEM_OCID"           --export-set-id "$export_set_ocid"           --query 'data[0].id'           --raw-output 2>/dev/null || true
      )"
    fi
  fi
